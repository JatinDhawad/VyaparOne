...
async def create_purchase_invoice(
    ...
    1. Calculates line item effective unit costs (without adding freight into unit cost).
    2. Calculates invoice totals: subtotal, tax_amount, additional_expenses (freight), grand_total.
    3. Saves PurchaseInvoice & PurchaseItem records.
    4. Updates GodownStock (current_stock & average_landed_cost).
    5. Posts double-entry ledger records (Purchase Account + Freight Expense Account -> Supplier Account).
    """
    ...
    for item_in in invoice_in.items:
        ...
        # Effective unit cost = net money spent for item / total items received (including free)
        if total_qty > 0:
        ...
        db_item = PurchaseItem(
            ...
            allocated_additional_cost=Decimal("0.00"),  # As per user directive: freight not allocated per item
            effective_unit_landed_cost=round(effective_unit_cost, 2),
            line_total=round(line_total, 2)
        )
        db_items.append((db_item, item_in.product_id, total_qty, round(effective_unit_cost, 2)))

    net_subtotal = subtotal - total_discount
    grand_total = net_subtotal + total_tax + freight_charges
    ...
    # 3. Associate items and update inventory
    ...
    await post_purchase_ledger(
        ...
        freight_amount=round(freight_charges, 2),
        grand_total=round(grand_total, 2),
        invoice_date=invoice_in.invoice_date,
        created_by=created_by
    )
